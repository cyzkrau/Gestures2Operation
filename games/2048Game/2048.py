    ...
    for pi in range(4):
        for pj in range(4):
            ...
            for point in range(4):
                if pi + x[point] > -1 and pi + x[point] < 4 and pj + y[
                        point] > -1 and pj + y[point] < 4 and TABLE[pi][
    ...
    for i in range(4):
        for j in range(4):
            pygame.draw.rect(screen, dictcolor1[TABLE[i][j]],
                             (j * boxsize + margin, i * boxsize + margin,
                              boxsize - 2 * margin, boxsize - 2 * margin),
                             thickness)
            if TABLE[i][j] != 0:
                order = int(math.log10(TABLE[i][j]))
                myfont = pygame.font.SysFont("Arial",
                                             fontSize[order],
                                             bold=True)
                ...
                screen.blit(
                    label,
                    (j * boxsize + 2 * margin, i * boxsize + 9 * margin))
...
def runGame(TABLE):
    ...
    show(TABLE)
    running = True
    hand_monitor = gesture.GestureMonitor(400, 200)

    while True:
        hand_monitor.update()
        hand_monitor.show()
        h = hand_monitor.move_h()
        if h != None:
            new_table = key(h, copy.deepcopy(TABLE))
            if new_table != TABLE:
            ...
            if gameOver(TABLE):
                showGameOverMessage()
        print(h)
        v = hand_monitor.move_v()
        if v != None:
            new_table = key(v, copy.deepcopy(TABLE))
            if new_table != TABLE:
                TABLE = randomfill(new_table)
                show(TABLE)
            if gameOver(TABLE):
